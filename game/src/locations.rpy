init python:
    ...
    mercury.add_clue("in the process of buying some extreme weather gear. Surface temperatures at their destination ranges from -173 to 427 Celsius")
    locations.append(mercury)

    venus = Location('Venus')
    venus.add_clue("hoping to visit the sister of Earth")
    venus.add_clue("going to a terrestrial planet that rotates counter-clockwise")
    ...
    mars.add_clue("a terrestrial planet with 2 moons")
    ...
    mars.add_clue("visiting a world first discovered by the Egyptians in the 2nd millennium BC")
    ...
    jupiter.add_clue("hoping to visit the largest planet in the solar system")
    ...
    def location_menu(loc, p_loc):
        ...
        menu = []
        menu.append(("Destination, Inquisitor?", None))
        for c in choices:
            if c.name == loc.name:
